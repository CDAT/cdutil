...
def centroid(msk, bounds, coords=None):
    ...
    if coords is None:
        # if MV2 then gets the bounds from there
        coords = msk.getAxis(0).getBounds()
    n = len(coords)
    ...
    for i in range(n):
        ...
        # factor to multiply by plus normalization
        w = w * c / float(bounds[1] - bounds[0])
        if isinstance(msk, numpy.ndarray):
            msk[i] = (1. - msk[i]) * w
        else:
            msk = (1. - msk) * w
...
def isMonthly(s):
    ...
    for i in range(len(tim) - 1):
        month1 = cdtime.reltime(
            tim[i],
            units).torel(
            'months since 2000',
            tim.getCalendar()).value
        month2 = cdtime.reltime(tim[i + 1],
                                units).torel('months since 2000',
                                             tim.getCalendar()).value
        if month2 - month1 != 1:
            monthly = 0
    return monthly
...
def mergeTime(ds, statusbar=1, fill_value=1.e20):
    ...
    for i in range(nslab):
        ...
        tmpdic = {}
        for ii in range(
                len(times[i])):  # creates the dictionary of times not matched yet
            t = times[i][ii]
            tmpval = cdtime.reltime(
                times[i][ii], units).torel(
                'seconds since 2000', cal)
            tmpdic[ii] = tmpval
    ...
    if len(numpy.unique(vals)) != nt:
        for i in range(nt - 1):
            if vals[i] == vals[i + 1]:
                errtime = cdtime.reltime(
                    vals[i], 'seconds since 2000').tocomp()
                err = 'Error in merging process : duplicate time point\n'
                err = err + \
                    str(errtime) + ' is duplicated, cannot merge' + \
                    str(vals[i - 2:i + 3])
                raise Exception(err)
            ...
            for i in range(nslab):
                ...
                for it in numpy.sort(list(timesleft[i].keys())):
                    ...
                    if val.value == vals[v]:
                        out[v] = ds[i][it]
                        fvals[v] = val.torel(
                            times[0].units, times[0].getCalendar()).value
                        bnds[v] = tim.getBounds()[it]
                        bnds[v][0] = switchCalendars(
                            bnds[v][0],
                            tim.units,
                            tim.getCalendar(),
                            times[0].units,
                            times[0].getCalendar()).value
                        bnds[v][1] = switchCalendars(
                            bnds[v][1],
                            tim.units,
                            tim.getCalendar(),
                            times[0].units,
...
def switchCalendars(t1, u1, c1, u2, c2=None):
    ...
    """
    if not (isinstance(t1, int) or isinstance(t1, float)):
        c2 = u2
        u2 = c1
        c1 = u1
        u1 = t1.units
...
class TimeSlicer:
    ...
    def get(self, slab, slicerarg=None, criteriaarg=None,
            statusbar=None, weights=False, sum=False):
        """
...
# raise Exception,'Error Slicer return nothing for: '+str(slicerarg)
    ...
    def departures(self, slab, slicerarg=None, criteriaarg=None,
                   ref=None, statusbar=None, sum=False):
        """
        ...
        if order[0] != 't':
            sliced = sliced(order='t...')
        order2 = sliced.getOrder(ids=1)
        weights = self.slicer(sliced.getTime(), slicerarg)
        if ref is None:
            if sum is False:
                ref = numpy.ma.average(
                    sliced, axis=0, weights=list(zip(*weights[2]))[0])
            else:
                ref = numpy.ma.sum(
                    sliced, axis=0, weights=list(zip(*weights[2]))[0])
        elif len(order2[1:]) > 0:
    ...
    def average(self, slab, slices, bounds, norm, criteriaarg=None,
                statusbar=None, weights=False, sum=False):
        """
...
# print j,w
            ...
            if len(sub) > 1:
                out[i] = numpy.ma.sum(
                    msk * slab[sub[0]:sub[-1] + 1].asma(), axis=0)
            ...
            if criteriaarg is not None:
                ...
                out[i] = self.criteria(
                    out[i], msk, [
                        norm[i][1], norm[i][1] + norm[i][0], ], criteriaarg)
    ...
    def statusbar1(self, i, n, statusbar):
        if statusbar is not None and n != 1:
            import genutil
            if not type(statusbar) in [type([]), type(())]:
                if not isinstance(statusbar, type('')):
                    status = float(i) / (n - 1.)
                else:
                    status = float(i + eval(statusbar)
                                   [0] * n) / (eval(statusbar)[1] * n)
            else:
                status = statusbar[1:]
                tot = eval(statusbar[0])
                if isinstance(tot, type('')):
                    status.insert(0, float(i + eval(statusbar)
                                           [0] * n) / (eval(statusbar)[1] * n))
                else:
                    status.insert(0, float(i) / (n - 1.))
            self.prev = genutil.statusbar(
                status, prev=self.prev, title=self.title)
            ...
            if not type(statusbar) in [type([]), type(())]:
                if not isinstance(statusbar, type('')):
                    if type(self.prev) in [type([]), type(())]:
                        if not isinstance(self.prev[0], type(0)):
                            self.prev[0].destroy()
                    self.prev = 0
                ...
                if isinstance(tot, type('')):
                    ...
                    if type(self.prev) in [type([]), type(())]:
                        if not isinstance(self.prev[0], type(0)):
                            self.prev[0].destroy()
                    self.prev = 0
            if type(statusbar) in [type([]), type(())]:
                statusbar.pop(0)
...
def dayBasedSlicer(tim, arg=None):
    ...
    for i in range(len(arg)):
        ...
        if isinstance(subarg, basestring):
            raise Exception(
                "Error, arguments to dayBasedSlicer must be strings")
        sp = subarg.split("-")
        if sp[0] == subarg:
            sp = subarg.split('/')
            if sp[0] == subarg:
                try:
                    ...
                    raise Exception(
                        "Error, dayBasedSlicer args must have '-' or '/' as month/day separator")
        try:
            day = int(sp[1])
            try:
            ...
            try:
                ...
                raise Exception(
                    "Error dayBasedSlicer couldn't understand argument: " + subarg)
...
def insert_monthly_seasons(data, seasons):
    ...
    for season in seasons:
        ...
        for i in range(data.shape[0] - 1):
            tscan = cdtime.reltime(
                tbnd[i][1], t.units).tocomp(
                t.getCalendar())  # end of current time step
            # beginnig of next time step
            tnext = cdtime.reltime(
                tbnd[i + 1][0], t.units).tocomp(t.getCalendar())
...
# print season,tb,te,tscan,tnext,tb.cmp(tscan)>-1, te.cmp(tnext)<1,tb.cmp(tscan)>-1 and te.cmp(tnext)<1
            ...
            while te.cmp(tnext) < 1:
                if tb.cmp(tscan) > -1:
                    ...
                    # ok sometimes we need to round up these things (months
                    # since especially)
...
# print 'N is:',n,'for season',season
    ...
    if adds != []:
        ...
        return mergeTime(adds, statusbar=None,
                         fill_value=getattr(data, 'fill_value', 1.e20))
    return data
...
class Seasons(ASeason):
    ...
    def get(self, slab, slicerarg=None,
            criteriaarg=None, statusbar=None, sum=False):
    ...
    def departures(self, slab, slicerarg=None, criteriaarg=None,
                   ref=None, statusbar=None, sum=False):
        """ Return the departures for the list of season you specified,
        returned in chronological order
        i.e. if you asked for DJF and JJA and the first season of your dataset is JJA
        you will have a JJA first !!!!
        Check your time axis coordinate !!!
        To pass a specific array from which to compute departures,
        ...
        for i in range(n):
            ...
            if statusbar is not None:
                if not type(statusbar) in [type([]), type(())]:
                    ...
                    statusbar.insert(
                        0, str([float(i), len(self.departures_seasons)]))
                else:
                    statusbar[0] = str(
                        [float(i), len(self.departures_seasons)])
            ...
            if ref is not None:
                ...
                if ref.shape[0] != len(self.departures_seasons):
                    ...
                    raise ValueError(
                        "The reference time (or first) dimension does not match the number of dimensions")
                newref = ref[i]
            else:
                newref = None
            # now computes the departures
            out = TimeSlicer.departures(
                self,
                slab,
                slicerarg=self.departures_seasons[i],
                criteriaarg=criteriaarg,
                statusbar=statusbar,
                ref=newref,
                sum=sum)
            # Adds it to the list
            if out is not None:
                s.append(out)
        self.seasons = self.departures_seasons
        if statusbar is not None and len(self.seasons) != 1:
            if type(statusbar) in [type([]), type(())]:
                statusbar.pop(0)
    ...
    def climatology(self, slab, criteriaarg=None,
                    criteriaargclim=None, statusbar=None, sum=False):
        ...
        ax = slab.getAxisList()
        ax[0] = t
        s = cdms2.createVariable(
            s, id=slab.id, copy=0, fill_value=getattr(
                slab, 'fill_value', 1.e20))
        s.setAxisList(ax)
        if initialgrid is not None:
